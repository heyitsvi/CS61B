...
public class Model extends Observable {
    ...
    public static boolean maxTileExists(Board b) {
        for (int col = 0; col < b.size(); col++){
            for (int row = 0; row < b.size(); row++){
                int tileValue;
                try{
                    tileValue = b.tile(col, row).value();
    ...
    public  static boolean checkAdjacentTileValues(Board b){
        for (int col = 0; col < b.size(); col++){
            for (int row = 0; row < (b.size() - 1); row++){
                int firstValue = b.tile(col, row).value();
                int secondValue = b.tile(col, row + 1).value();
        ...
        for (int row = 0; row < b.size(); row++){
            for (int col = 0; col < (b.size() - 1); col++){
                int firstValue = b.tile(col, row).value();
                int secondValue = b.tile(col + 1, row).value();
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        if (emptySpaceExists(b) || checkAdjacentTileValues(b)){
            return true;
        }
